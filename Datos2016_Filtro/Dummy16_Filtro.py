...
# Modificación del DataFrame para quedarnos con los datos del año 2016
df16 = df.iloc[:, 72:92]
dfaux = df.iloc[:, 100:103] # Columnas GBA16, ITG16 y PrediabHb16
df16 = pd.concat([df16, dfaux], axis=1)

# Creación de la columna Edad16
df16['Edad16'] = df['FechNac'].apply(lambda año: 2016 - año)

# Concatenamos DataFrame del año 2010 con el del año 2016
df16 = pd.concat([df10, df16], axis=1)

# Eliminamos aquellas filas que los pacientes hayan fallecido y no tengan datos en el año 2016, 
# utlizamos la columna TAS116 por ejemplo
df16 = df16.dropna(subset=['TAS116'])
...
df16 = df16[(df16["Muerte_SN"] != 1) | (df16["Motivo1"].isin(motivos))]

# Eliminación de las columnas que no son necesarias
df16 = df16.drop(columns=['Tseguim', 'MuerteCV_SN', 'MuerteCANCER_SN',
...
df16[df16.columns] = imputer_knn.fit_transform(df16[df16.columns])

# Se reordena el DataFrame para que la clase este en la última columna
colum_clase = 2
columnas = df16.columns[:colum_clase].to_list() + df16.columns[(colum_clase+1):].to_list() + df16.columns[colum_clase:(colum_clase+1)].to_list()
df16 = df16[columnas]

# Eliminación de las columnas que no son necesarias
df16 = df16.drop(columns=['FechNac'])
df16['Edad16'] = df16['Edad16'].astype(int)

# DataFrame con todos los años preparado
print(df16)
