...
# Creamos el SVC con kernel polinómico (class_weight ---> None (con Scaler) / balanced (sin Scaler))
svc_poli = Pipeline([('std', StandardScaler()), 
                    ('poli', SVC(kernel='poly', random_state=1234, class_weight = None))])

# Grid para la búsqueda de hiperparámetros
param_grid = {
    'poli__C': [0.01, 0.1, 1, 10, 100],
    'poli__degree' : [2, 3, 4]
}
