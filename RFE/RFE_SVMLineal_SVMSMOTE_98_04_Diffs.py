...
# Creación de los nuevos atributos con las diferencias entre 1998 y 2004
df04['Peso_Diff'] = df04['Peso04'] - df04['Peso98']
df04['IMC_Diff'] = df04['IMC04'] - df04['IMC98']
df04['CT_Diff'] = df04['CT04'] - df04['CT98']
df04['HDL_Diff'] = df04['HDL04'] - df04['HDL98']
df04['TG_Diff'] = df04['TG04'] - df04['TG98']
df04['LDL_Diff'] = df04['LDL04'] - df04['LDL98']
df04['Cr_Diff'] = df04['Cr04'] - df04['Cr98']
df04['Hba1c_Diff'] = df04['HbA1c04'] - df04['Hba1c98dc']
df04['Urea_Diff'] = df04['Urea04'] - df04['Urea98']
df04['GBA_Diff'] = df04['GBA04'] - df04['GBA98']
df04['ITG_Diff'] = df04['ITG04'] - df04['ITG98']
...
svc_lin = SVC(kernel='linear', random_state=1234, class_weight = None)

# Creamos el sampler y lo aplicamos a X e y
sampler = SVMSMOTE(sampling_strategy = 0.7, random_state=42)
sampler_X, sampler_y = sampler.fit_resample(X, y)

# Escalado solo para RFE
scaler = StandardScaler()
X_scaled = scaler.fit_transform(sampler_X)

# Ranking de los atributos de mayor a menor importancia
rfe = RFE(svc_lin, n_features_to_select=1, step=1)
rfe.fit(X_scaled, sampler_y)
ranking = np.argsort(rfe.ranking_)
columnas_ordenadas = X.columns[ranking]
print("\nRanking de los atributos:")
print(columnas_ordenadas)

# Aplicamos RFE a cada sistema que queremos evaluar y con todos los posibles atributos
for i in range(len(columnas_ordenadas), 0, -1):
      
    print("\nAtributo número: %d" % (i))
    atributos_selec = columnas_ordenadas[:i]
    print("Atributos seleccionados (%d):" % len(atributos_selec))

    # Creamos el conjunto X con los atributos seleccionados por RFE
    X_rfe = X[atributos_selec]
    print(X_rfe.columns)

    # Creamos un pipeline con los métodos de OverSampling, UnderSampling y el sistema que queramos
    pipeline = Pipeline([
        ("oversampling", SVMSMOTE(sampling_strategy = 0.7, random_state=42)),
        #("oversampling", RandomOverSampler(sampling_strategy = 0.7, random_state=42)),
        #("modelo", SVC(kernel='poly', random_state=1234, class_weight = None))
        ("modelo", SVC(kernel='linear', random_state=1234, class_weight = None))
    ])

    # GridSearchCV con validación cruzada de 5 folds (Optimización --->  Sobre F1 o FBeta-Score)
    grid_search = GridSearchCV(pipeline, param_grid_lin, cv=folds5, scoring = 'f1', verbose=1, n_jobs=1)

    # Obtenemos resultados con validación cruzada
    scores = cross_validate(grid_search, X_rfe, y, cv=folds10, scoring=scoring, return_estimator=True, verbose=1,  n_jobs=-1)
