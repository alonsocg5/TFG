...
log_reg = Pipeline([('std', StandardScaler()), 
                    ('lr', LogisticRegression(random_state=1234, max_iter=200, class_weight = None))])

# Grid para la búsqueda de hiperparámetros
param_grid = {
    'lr__C':  np.logspace(1, 4, 10)     # de 10^1 (10) a 10^4 (10000), 10 valores
}

# GridSearchCV con validación cruzada de 5 folds (Optimización --->  Sobre FBeta-Score)
grid_search = GridSearchCV(log_reg, param_grid, cv=folds5, scoring = fbeta_scorer, verbose=1, n_jobs=-1)
