...
param_grid = {
    'n_estimators': [25, 50, 100],          # Número de árboles
    'max_depth': range(3, 15),              # Profundidad máxima de los árboles
    'min_samples_split': [2, 3, 5, 8 ,10],  # Mínimo número de muestras para dividir un nodo
}

# GridSearchCV con validación cruzada de 5 folds (Optimización --->  Sobre FBeta-Score)
grid_search = GridSearchCV(rf, param_grid, cv=folds5, scoring = fbeta_scorer, verbose=1, n_jobs=-1)
...
best_index = np.argmax(scores['test_fbeta'])  # Índice del mejor modelo según FBeta
