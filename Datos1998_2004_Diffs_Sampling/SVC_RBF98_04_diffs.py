...
svc_rbf = Pipeline([('std', StandardScaler()), 
                    ('rbf', SVC(kernel='rbf', random_state=1234, class_weight = None))])

# Grid para la búsqueda de hiperparámetros
param_grid = {
    'modelo__rbf__C': [0.01, 0.1, 1, 10, 100],
    'modelo__rbf__gamma' : [1e-4, 1e-3, 1e-2, 1e-1, 1, 10, 100]
}

# Creamos un pipeline con los métodos de OverSampling, UnderSampling y el sistema que queramos
pipeline = Pipeline([
    ("oversampling", ADASYN(sampling_strategy = 0.5, random_state=42)),
